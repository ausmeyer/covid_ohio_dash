...
ui <- fluidPage(
    ...
    "Charts will build automatically 1.5 seconds after changing any parameter. 
        ...
        The Data menu provides a selection of the available from the Ohio Department of Health; the data will also display on the map.
...
server <- function(input, output, session) {
    ...
    observe({
        inputData()
        if(input$transformation == 'none' | !as.logical(input$align)) {
    ...
    observe({
        inputData()
        if(length(input$countyChoice) != 1 | length(input$sex) != 1) {
            updatePickerInput(session, "ageRange",
                              selected = list('Total'))
        }
    })
    
    observe(shinyjs::toggleState("ageRange", 
                                 (length(input$countyChoice) == 1) & length(input$sex) == 1))
